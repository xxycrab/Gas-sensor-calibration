...
def test_SdA(pre_run,kind, PV,true_out, h_activation=[],
             finetune_lr=0.1, pretraining_epochs=15,
             pretrain_lr=0.001, training_epochs=1000,
             datasets='mnist.pkl.gz', batch_size=1,
             hidden_layers_sizes=[1000, 1000, 1000],
             input_num=784, out_num=10,name="",path=""):
    """
    ...
    array_PV = PV
    sda.PV.set_value(0.5*numpy.ones(PV.shape,dtype="float32")) 

    while (epoch < training_epochs) and (not done_looping):
        epoch = epoch + 1
        if epoch == pre_run:
            sda.PV.set_value(array_PV)
 
        for minibatch_index in xrange(n_train_batches):

            tem = train_fn(minibatch_index)
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    test_score = numpy.mean(test_losses[0])
    ...
    test_out = test_model()
    test_tem = numpy.asarray(test_out[1]).reshape((n_test_batches * batch_size,out_num))
...
def build_sda(kind, PV,true_out ,path, datasets, name):
