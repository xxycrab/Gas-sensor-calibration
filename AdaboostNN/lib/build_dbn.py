...
def test_DBN(pre_run,kind, PV,true_out, h_activation=[],finetune_lr=0.1, pretraining_epochs=10,
             pretrain_lr=0.01, k=1, training_epochs=1000,
             datasets='mnist.pkl.gz', batch_size=256,
             input_num=784,hidden_layers_sizes=[1000, 1000, 1000],
             out_num=10,path="",name=""):
    """
    ...
    dbn = DBN(numpy_rng=numpy_rng, n_ins=input_num, h_activation  = h_activation,
              hidden_layers_sizes=hidden_layers_sizes,n_outs=out_num,kind = kind,
              PV = PV)
    ...
    array_PV = PV
    dbn.PV.set_value(0.5*numpy.ones(PV.shape,dtype="float32")) 

    while (epoch < training_epochs) and (not done_looping):
        epoch = epoch + 1
        if epoch == pre_run:
            dbn.PV.set_value(array_PV)
    ...
    test_out = test_model()[1]
    test_tem = numpy.asarray(test_out).reshape((n_test_batches * batch_size,out_num))
